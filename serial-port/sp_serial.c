...
sp_serial_open (char *name)
{
    ...
    printf ("Port is open");
    obj = (void *)::malloc (sizeof (struct port_s));
...
int
sp_configure_device (void *obj, int Baudrate, int databits, int stopbit,
		     int parity, int flow, int timeout)
{
...
int
sp_write_device (void *obj, char *data, int len)
{
    struct port_s *port = (struct port_s *) obj;
    int fd = port->fd;
    int ret;
...
int
sp_read_device (void *obj, char *buf, int len)
{
    struct port_s *port = (struct port_s *) obj;
    int fd = port->fd;
    int ret;
...
main ()
{
    ...
    if (0 == stat)
	  ...
	  printf
	      ("Enter the config parameters: baudrate, databits, stopbits, parity, flow and timeout\n");
	  scanf ("%d %d %d %c", &baudrate, &databits, &stopbits, &parity);
	  sp_configure_device (sp_serport, baudrate, databits, stopbits,
			       parity);
	  while (1)
	    {
